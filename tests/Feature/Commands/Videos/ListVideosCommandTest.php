    ...
    $this->artisan(ListVideosCommand::class)
        ->expectsTable([
            ...
            return [
                $video->title,
                $video->channel->name,
                Date::parse($video->published_at)->diffForHumans(),
                $video->notified_at ? Date::parse($video->notified_at)->diffForHumans() : '—',
                $video->getYoutubeUrl(),
            ];
        })->toArray());
});
    ...
    $this->artisan(ListVideosCommand::class)
        ->expectsTable([
            ...
            [
                ...
                Date::parse($newVideo->published_at)->diffForHumans(),
                $newVideo->notified_at ? Date::parse($newVideo->notified_at)->diffForHumans() : '—',
                $newVideo->getYoutubeUrl(),
            ],
            [
                $middleVideo->title,
                $middleVideo->channel->name,
                Date::parse($middleVideo->published_at)->diffForHumans(),
                $middleVideo->notified_at ? Date::parse($middleVideo->notified_at)->diffForHumans() : '—',
                $middleVideo->getYoutubeUrl(),
            ],
            [
                $oldVideo->title,
                $oldVideo->channel->name,
                Date::parse($oldVideo->published_at)->diffForHumans(),
                $oldVideo->notified_at ? Date::parse($oldVideo->notified_at)->diffForHumans() : '—',
    ...
    $this->artisan(ListVideosCommand::class, ['--notified' => true])
        ->expectsTable([
            ...
            [
                ...
                Date::parse($notifiedVideo->published_at)->diffForHumans(),
                Date::parse($notifiedVideo->notified_at)->diffForHumans(),
                $notifiedVideo->getYoutubeUrl(),
            ],
            [
                $anotherNotifiedVideo->title,
                $anotherNotifiedVideo->channel->name,
                Date::parse($anotherNotifiedVideo->published_at)->diffForHumans(),
                Date::parse($anotherNotifiedVideo->notified_at)->diffForHumans(),
