...
it('returns the full youtube link', function () {
    $videoId = '1234566789';
...
it('returns thumbnail urls with different qualities', function () {
    $videoId = 'abc12345';
    $video = Video::factory()->create(['video_id' => $videoId]);
...
it('formats published date for human-readable display', function () {
...
it('formats published date as ISO8601 for Discord', function () {
    // Create a video with a fixed publish date
    $publishDate = Carbon::create(2023, 5, 15, 14, 30, 0);
    $video = Video::factory()->create([
        'published_at' => $publishDate,
